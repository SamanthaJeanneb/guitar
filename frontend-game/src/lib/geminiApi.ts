...
export async function generateChartFromSongName(songName: string): Promise<string> {
  try {
...
// Generic function to make any Gemini API request
export async function makeGeminiRequest(prompt: string, modelName: string = "gemini-2.5-pro"): Promise<string> {
  try {
...
export function parseChartMetadata(chartContent: string) {
  const metadata = {
  ...
  try {
    ...
    console.log('Error parsing chart metadata:', error);
  }
